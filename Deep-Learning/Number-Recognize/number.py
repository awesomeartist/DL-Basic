...
trainset = datasets.MNIST(root = './Dataset/mnist/', train = True, download = False, transform = transform)    #下载训练集存放在当前data文件夹中

trainsetloader = torch.utils.data.DataLoader(trainset, batch_size = 100, shuffle = True)           #batch表示一次性喂给模型多少数据

testset = datasets.MNIST(root= './Dataset/mnist/', train = False, download = False, transform = transform)      #下载测试集存放在当前data文件夹中
...
# 保存下训练好的模型,省得下次再重新训练
torch.save(model,'./Number-Recognize/my_handwrite_recognize_model.pt')
